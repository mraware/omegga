...
class Omegga extends OmeggaWrapper {
  ...
  /**
   * emits start, does final startup setups
   * this should not be called by a plugin
   * @returns {Promise}
   */
  //
  async prestart() {
    // TODO: Remove this when we can set map on startup
    if (this.config.server && this.config.server.map && this.config.server.map.toLowerCase() !== this.currentMap) {
      await this.changeMap(this.config.server.map);
    }
    this.emit('start', this.currentMap);
  }
